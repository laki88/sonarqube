...
public class ShortBranchTrackerExecutionTest {
  ...
  public void simple_tracking() {
    ...
    ShortBranchTracking tracking = underTest.track(FILE);
    assertThat(tracking.getUnmatchedBases()).isEmpty();
    assertThat(tracking.getMatchedWithBase()).isEmpty();
    assertThat(tracking.getMatchedWithMergeBranch()).isEmpty();
  ...
  public void tracking_with_all_results() {
    ...
    mergeBranchIssues.add(rawIssues.get(1));

    ShortBranchTracking tracking = underTest.track(FILE);
    assertThat(tracking.getUnmatchedBases()).isEmpty();
    assertThat(tracking.getMatchedWithBase()).isEqualTo(Collections.singletonMap(rawIssues.get(0), rawIssues.get(0)));
    assertThat(tracking.getMatchedWithMergeBranch()).isEqualTo(Collections.singletonMap(rawIssues.get(1), rawIssues.get(1)));
